...
class StreamServerController extends Controller
{
    ...
    public function searchMod(Request $request, $limit_request)
    {
        ...
        foreach ($json->models as $key => $mod) {
            ...
            // dd($mod, $json_description->user);
            // $json->models[$key]->description = $description;
            // $json->models[$key]->especification = $json_description->user->user;
            // if (strpos($description, "Hello Guys! ... ") !== false) {
            //     array_push($model_find, $json_description->user->user->username);
            // }
        }
